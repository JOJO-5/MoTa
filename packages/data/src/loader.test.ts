...
describe('loadTowerContent', () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(join(tmpDir, 'enemys.json'), JSON.stringify({ greenSlime: { id: 'greenSlime', name: '史莱姆', hp: 100, atk: 10, def: 10, money: 5, exp: 2 } }))
    await writeFile(join(tmpDir, 'maps.json'), JSON.stringify({ '0': { id: '0', name: '地板' } }))
    await writeFile(join(tmpDir, 'items.json'), JSON.stringify({ redKey: { id: 'redKey', name: '红钥匙' } }))
    await writeFile(join(tmpDir, 'events.json'), JSON.stringify({}))

    const map13 = Array(13).fill(null).map(() => Array(13).fill(0))
    await writeFile(join(tmpDir, 'floors', 'MT0.json'), JSON.stringify({ floorId: 'MT0', title: '一层', name: '第1层', width: 13, height: 13, map: map13 }))
    await writeFile(join(tmpDir, 'floors', 'MT1.json'), JSON.stringify({ floorId: 'MT1', title: '二层', name: '第2层', width: 13, height: 13, map: map13 }))
  })
