var functions_d6ad677b_427a_4623_b50f_a445a3b0ef8a = 
{
	...
	if (core.getFlag('pzf', 0) == 0) core.status.hero.hp = 2 * core.getRealStatus('atk') + 2 * core.getRealStatus('def') + 500 * core.itemCount('bomb');
	...
	if (core.enemys.hasSpecial(special, 3)) {
		var animate = 'mallet'; // 此为拥有重甲防御时用的特殊攻击动画
	} else {
		var animate = 'hand'; // 默认动画
		// 检查当前装备是否存在攻击动画
	...
	*/
	...
	return [
		[1, "疾速攻击", "怪物首先攻击", "#ffcc33"],
		[2, "魔法攻击", "怪物无视角色的防御", "#bbb0ff"],
		[3, "重甲防御", "怪物防御不小于角色攻击-1", "#c0b088"],
		[4, "2回攻击", "怪物每回合攻击2次", "#ffee77"],
		[5, "3回攻击", "怪物每回合攻击3次", "#ffee77"],
		[6, function (enemy) { return (enemy.n || '') + "回攻击"; }, function (enemy) { return "怪物每回合攻击" + (enemy.n || 4) + "次"; }, "#ffee77"],
		[7, "爆裂攻击", function (enemy) { return "战斗前，怪物附加角色防御的" + Math.floor(100 * (enemy.breakArmor || core.values.breakArmor || 0)) + "%作为伤害"; }, "#88c0ff"],
		...
		[9, "死气邪术", function (enemy) { return "战斗前，怪物附加角色护盾的" + (enemy.purify || core.values.purify) + "倍作为伤害"; }, "#80eed6"],
		[10, "影子法术", "怪物的攻防和角色攻防相关", "#b0c0dd"],
		[11, "血魔法", function (enemy) { return "战斗前，怪物首先吸取角色的" + Math.floor(100 * enemy.vampire || 0) + "%生命（约" + Math.floor((enemy.vampire || 0) * core.getStatus('hp')) + "点）作为伤害" + (enemy.add ? "，并把伤害数值加到自身生命上" : ""); }, "#dd4448"],
		[12, "毒攻击", "战斗后，角色陷入中毒状态，每一步损失生命" + core.values.poisonDamage + "点", "#99ee88"],
		[13, "衰弱攻击", "战斗后，角色陷入衰弱状态，攻防暂时下降" + (core.values.weakValue >= 1 ? core.values.weakValue + "点" : parseInt(core.values.weakValue * 100) + "%"), "#f0bbcc"],
		[14, "诅咒", "战斗后，角色陷入诅咒状态，战斗无法获得金币和经验", "#bbeef0"],
		[15, "魔法阵", function (enemy) { return "经过怪物周围" + (enemy.zoneSquare ? "九宫格" : "十字") + "范围内" + (enemy.range || 1) + "格时自动减生命" + (enemy.zone || 0) + "点"; }, "#c677dd"],
		[16, "守卫阵", "经过两只相同的怪物中间，角色生命值变成一半", "#bb99ee"],
		[17, "仇恨", "战斗前，怪物附加之前积累的仇恨值作为伤害；战斗后，释放一半的仇恨值。（每杀死一个怪物获得" + (core.values.hatred || 0) + "点仇恨值）", "#b0b666"],
		[18, "魔法阵", function (enemy) { return "经过怪物周围" + (enemy.zoneSquare ? "九宫格" : "十字") + "时自动减生命" + (enemy.repulse || 0) + "点，同时怪物后退一格"; }, "#8888e6"],
		[19, "自我毁灭", "战斗后角色直接死亡", "#ff6666"],
		[20, "神秘诅咒", "角色在不持有十字架的情况下无法打败这类怪物", "#aaaaaa"],
		...
		[23, "不死身", "怪物被击败后，角色转换楼层则怪物将再次出现", "#a0e0ff"],
		...
		[28, "镜像攻击", "敌方攻击跟角色攻击相同", "#b0c0dd"],
		[29, "自我毁灭", "战斗后角色直接死亡", "#ff6666"],
		[30, "狂暴攻击", function (enemy) { return "怪物每回合固定增加" + (enemy.value || 0) + "点伤害（可被护盾抵消）"; }, "#ff6666"],
		[31, "暗属性", "角色防御力比怪物攻击力低时，怪物对角色造成的伤害三倍化", "#999999"],
		[32, "再生能力", "怪物被击败后，会以另一种形态再次出现", "#a0e0ff"]
	];
},
	...
	if (core.hasSpecial(mon_special, 10)) {
		mon_atk = hero_atk * 1.3;
		mon_def = hero_def * 0.6;
	}
	// 坚固
	...
	// 镜像攻击
	if (core.hasSpecial(mon_special, 28) && mon_atk < hero_atk) {
		mon_atk = hero_atk;
	}
	...
	if (core.hasSpecial(mon_special, 31)) {
		if (hero_def < mon_atk) {
			damage *= 3; //暗属性状态下若角色防御不敌怪物攻击，造成的总伤害三倍化
		}
	}
	...
	for (var loc in blocks) {
		...
		if (id == 'lavaNet' && !core.hasItem('amulet')) {
			...
			type[loc][(block.event.name || "熔岩") + "伤害"] = true;
		}
	...
	*/
	...
	var ctx, fill = function (text, x, y, style) {
		core.ui.setFont(ctx, (/\w+/.test(text) ? 'italic ' : '') + 'bold 14px hemi');
	...
	if (core.flags.statusCanvas) { // 系统开关「自绘状态栏」开启
		...
		if (core.domStyle.isVertical) { // 竖屏
			...
			core.drawImage(ctx, "state_m.png", 0, 0, 480, 137);
			//core.drawImage(ctx, core.statusBar.icons.floor, 6, 6, 25, 25);
			fill((core.status.thisMap || {}).name || "Loading", 64, 37);
			//core.drawImage(ctx, core.statusBar.icons.hp, 137, 6, 25, 25);
			fill(core.formatBigNumber(core.getRealStatus("lv")), 220, 37);
			if (core.hasFlag("poison")) {
				fill(core.formatBigNumber(core.getRealStatus('hp')), 374, 36, '#66CC99');
			} else {
				fill(core.formatBigNumber(core.getRealStatus('hp')), 374, 36, '#FFFFFF');
			}
			//core.drawImage(ctx, core.statusBar.icons.atk, 268, 6, 25, 25);
			if (core.hasFlag('weak')) {
				fill(core.formatBigNumber(core.getRealStatus('atk')), 64, 75, '#FF99CC');
				fill(core.formatBigNumber(core.getRealStatus('def')), 220, 75, '#FF99CC');
			} else {
				fill(core.formatBigNumber(core.getRealStatus('atk')), 64, 75, '#FFFFFF');
				fill(core.formatBigNumber(core.getRealStatus('def')), 220, 75, '#FFFFFF');
			}
			//core.drawImage(ctx, core.statusBar.icons.mdef, 137, 38, 25, 25);
			fill(core.formatBigNumber(core.getRealStatus('mdef')), 373, 75, '#FFFFFF');
			//core.drawImage(ctx, core.statusBar.icons.money, 268, 38, 25, 25);
			fill(core.formatBigNumber(core.status.hero.exp), 64, 116, '#FFFFFF');
			fill(core.formatBigNumber(core.status.hero.money), 374, 116, '#FFFFFF');
			fill(core.setTwoDigits(core.itemCount('yellowKey')), 180, 116, '#FFCCAA');
			fill(core.setTwoDigits(core.itemCount('blueKey')), 217, 116, '#AAAADD');
			fill(core.setTwoDigits(core.itemCount('redKey')), 254, 116, '#FF8888');
		} else if (!core.flags.hideLeftStatusBar) { // 横屏且未隐藏状态栏
			core.drawImage(ctx, "state.png", 0, 0, 149, 480);
			ctx.textAlign = 'center';
			//core.drawImage(ctx, core.statusBar.icons.floor, 6, 9, 25, 25);
			fill((core.status.thisMap || {}).name || "Loading", 76, 42);
			fill(core.formatBigNumber(core.getRealStatus("lv")), 76, 86);
			if (core.hasFlag("poison")) {
				fill(core.formatBigNumber(core.getRealStatus('hp')), 76, 128, '#66CC99');
			} else {
				fill(core.formatBigNumber(core.getRealStatus('hp')), 76, 128, '#FFFFFF');
			}
			if (core.hasFlag('weak')) {
				fill(core.formatBigNumber(core.getRealStatus('atk')), 76, 172, '#FF99CC');
				fill(core.formatBigNumber(core.getRealStatus('def')), 76, 215, '#FF99CC');
			} else {
				fill(core.formatBigNumber(core.getRealStatus('atk')), 76, 172, '#FFFFFF');
				//core.drawImage(ctx, core.statusBar.icons.def, 6, 111, 25, 25);
				fill(core.formatBigNumber(core.getRealStatus('def')), 76, 215, '#FFFFFF');
				//core.drawImage(ctx, core.statusBar.icons.mdef, 6, 145, 25, 25);
			}
			//core.drawImage(ctx, core.statusBar.icons.mdef, 6, 145, 25, 25);
			fill(core.formatBigNumber(core.getRealStatus('mdef')), 76, 258, '#FFFFFF');
			//core.drawImage(ctx, core.statusBar.icons.money, 6, 179, 25, 25);
			fill(core.formatBigNumber(core.status.hero.money), 76, 339, '#FFFFFF');
			//core.drawImage(ctx, core.statusBar.icons.exp, 6, 213, 25, 25);
			fill(core.formatBigNumber(core.status.hero.exp), 76, 298);
			fill(core.setTwoDigits(core.itemCount('yellowKey')), 35, 360, '#FFCCAA');
			fill(core.setTwoDigits(core.itemCount('blueKey')), 73, 360, '#AAAADD');
			fill(core.setTwoDigits(core.itemCount('redKey')), 113, 360, '#FF8888');
		}
	...
	core.fillText('ui', "魔塔2014", text_start, top + 35, globalAttribute.selectColor, "bold 22px " + globalAttribute.font);
	core.fillText('ui', "版本： " + main.__VERSION__, text_start, top + 80, "#FFFFFF", "bold 17px " + globalAttribute.font);
	core.fillText('ui', "作者：史莱姆魔皇", text_start, top + 112);
	core.fillText('ui', '使用样板：HTML5魔塔样板2.10.3', text_start, top + 112 + 32);
	core.fillText('ui', 'HTML5魔塔交流群：539113091', text_start, top + 112 + 64);
